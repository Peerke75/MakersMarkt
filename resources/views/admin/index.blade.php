...
<div class="container mx-auto py-8">
    ...
    <div class="tabs mb-6">
        <ul class="flex border-b">
            <li class="tab-item flex-1 text-center border-b-2 border-transparent py-2 px-4 cursor-pointer hover:text-blue-500 hover:border-blue-500" data-tab="productbeheer">
                Productenbeheer
            </li>
            <li class="tab-item flex-1 text-center border-b-2 border-transparent py-2 px-4 cursor-pointer hover:text-blue-500 hover:border-blue-500" data-tab="gebruikersbeheer">
                Gebruikersbeheer
            </li>
            <li class="tab-item flex-1 text-center border-b-2 border-transparent py-2 px-4 cursor-pointer hover:text-blue-500 hover:border-blue-500" data-tab="ongepaste-taal">
                Ongepaste Taal
            </li>
        </ul>
    </div>
                            ...
                            <tr>
                                ...
                                <td class="border-b px-4 py-2">
                                    ...
                                    @if ($product->status === 'pending' || $product->status === 'inactive')
                                        <form action="{{ route('admin.products.activate', $product->id) }}" method="POST" class="inline-block">
                                            @csrf
                                            @method('PATCH')
                                            <button type="submit" class="bg-green-500 text-white px-4 py-2 rounded">Activeren</button>
                                        </form>
                                    @endif

                                    @if ($product->status === 'active')
                                        <form action="{{ route('admin.products.deactivate', $product->id) }}" method="POST" class="inline-block">
                                            @csrf
                                            @method('PATCH')
                                            <button type="submit" class="bg-yellow-500 text-white px-4 py-2 rounded">Deactiveren</button>
                                        </form>
                                    @endif
